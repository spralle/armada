...
class FakeRoot {
  ...
  constructor(private readonly splitter: FakeDockSplitter) {}
...
export function registerDockSplitterDragSpecs(harness: SpecHarness): void {
  ...
  test("dock splitter drag updates horizontal ratio by clientX delta", () => {
    ...
    const root = new FakeRoot(splitter);
    ...
    splitter.dispatch("pointermove", pointerEvent({ pointerId: 1, clientX: 650, clientY: 10 }));
    splitter.dispatch("pointerup", pointerEvent({ pointerId: 1, clientX: 650, clientY: 10 }));
  ...
  test("dock splitter drag uses vertical clientY axis for vertical orientation", () => {
    ...
    splitter.dispatch("pointermove", pointerEvent({ pointerId: 2, clientX: 900, clientY: 350 }));
    splitter.dispatch("pointerup", pointerEvent({ pointerId: 2, clientX: 900, clientY: 350 }));
  ...
  test("dock splitter drag clamps ratio and ignores non-primary pointer button", () => {
    ...
    splitter.dispatch("pointermove", pointerEvent({ pointerId: 3, clientX: 900, clientY: 0 }));
    ...
    splitter.dispatch("pointermove", pointerEvent({ pointerId: 4, clientX: 1600, clientY: 0 }));
    splitter.dispatch("pointerup", pointerEvent({ pointerId: 4, clientX: 1600, clientY: 0 }));
  ...
  test("dock splitter drag cleanup runs on pointercancel and lostpointercapture", () => {
    ...
    splitter.dispatch("pointercancel", pointerEvent({ pointerId: 11, clientX: 500, clientY: 0 }));
