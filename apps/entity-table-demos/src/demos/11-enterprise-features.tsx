...
const LEADS = [
  'Alice Johnson', 'Bob Martinez', 'Carol Chen', 'David Kim', 'Eva Rossi',
  'Frank Okafor', 'Grace Liu', 'Hiro Tanaka', 'Isla Patel', 'James Wright',
  'Keiko Sato', 'Liam Brown', 'Mia Fernandez', 'Noah Andersen', 'Olivia Singh',
];
const PROJECT_NAMES = [
  'Platform Migration', 'Mobile Redesign', 'API Gateway', 'Data Pipeline',
  'Auth Service', 'Search Engine', 'Analytics Dashboard', 'CI/CD Overhaul',
  'Design System', 'Billing Module', 'Notification Hub', 'Compliance Audit',
  'Performance Tuning', 'Onboarding Flow', 'Partner Portal', 'Inventory Sync',
  'Chat Integration', 'Report Builder', 'SSO Federation', 'Edge Caching',
];
const TAG_POOL = [
  'frontend', 'backend', 'infra', 'security', 'ux', 'data', 'mobile',
  'devops', 'compliance', 'performance', 'api', 'testing', 'docs',
];
...
function generateProjects(count: number): Project[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    projects.push({
      ...
      name: PROJECT_NAMES[i % PROJECT_NAMES.length] + (i >= PROJECT_NAMES.length ? ` v${Math.floor(i / PROJECT_NAMES.length) + 1}` : ''),
      lead,
...
const rowOps: EntityOperation<Project>[] = [
  { id: 'view', label: 'View Details', icon: <Eye className="h-4 w-4" />, handler: ({ entity }) => alert(`View: ${entity?.name}`) },
  { id: 'edit', label: 'Edit Project', icon: <Pencil className="h-4 w-4" />, handler: ({ entity }) => alert(`Edit: ${entity?.name}`) },
  { id: 'delete', label: 'Archive', icon: <Trash2 className="h-4 w-4" />, variant: 'destructive', handler: ({ entity }) => alert(`Archive: ${entity?.name}`), when: ({ entity }) => entity?.status !== 'completed' },
];
...
export function EnterpriseFeaturesDemo() {
  return (
    <DemoShell
      ...
      features={['Column Filters', 'Column Resize', 'Sticky Header', 'Density Toggle', 'Multi-Sort', 'Row Selection', 'Row Actions']}
      schema={schemaSource}
