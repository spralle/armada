  ...
  test('extracts variant fields from oneOf', () => {
    const schema: JsonSchema = {
      ...
      properties: {
        pet: {
          oneOf: [
            { type: 'object', properties: { type: { type: 'string', const: 'cat' }, meows: { type: 'boolean' } }, required: ['type'] },
            { type: 'object', properties: { type: { type: 'string', const: 'dog' }, barks: { type: 'boolean' } }, required: ['type'] },
          ],
        },
      },
    };
...
describe('metadata extraction — constraints and annotations', () => {
  ...
  test('extracts dependentRequired', () => {
    const schema: JsonSchema = {
      type: 'object',
      properties: {
        country: { type: 'string', dependentRequired: { country: ['state'] } as unknown as Record<string, readonly string[]> },
      },
    };
