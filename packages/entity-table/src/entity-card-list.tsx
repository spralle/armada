...
import type { TableFieldDescriptor, TableFieldOverride, CompileTableFieldsOptions } from '@ghost-shell/table-from-schema'
...
function partitionFields(fields: TableFieldDescriptor[], slots?: Record<string, CardSlot>) {
  const result: Record<CardSlot | 'optional', TableFieldDescriptor[]> = {
    header: [], leading: [], body: [], trailing: [], footer: [], optional: [],
  }
...
function CardLeadingSlot<TData extends Record<string, unknown>>({ fields, row }: { fields: TableFieldDescriptor[]; row: TData }) {
...
function CardCenterSlot<TData extends Record<string, unknown>>({ header, body, row }: { header: TableFieldDescriptor[]; body: TableFieldDescriptor[]; row: TData }) {
  return (
    <div className="min-w-0 flex-1">
      {header.length > 0 && (
        <div className="space-y-0.5">
          {header.map((field, i) => (
            <div key={field.field} className={cn(i === 0 ? "text-base font-semibold leading-tight" : "text-sm text-muted-foreground")}>
              {formatValue(getFieldValue(row, field.field), field)}
            </div>
          ))}
        </div>
...
function CardTrailingSlot<TData extends Record<string, unknown>>({ fields, row }: { fields: TableFieldDescriptor[]; row: TData }) {
  if (fields.length === 0) return null
  return (
...
function CardFooterSlot<TData extends Record<string, unknown>>({ fields, row }: { fields: TableFieldDescriptor[]; row: TData }) {
  if (fields.length === 0) return null
  return (
...
function CardOptionalSlot<TData extends Record<string, unknown>>({ fields, row, expanded, onToggle }: { fields: TableFieldDescriptor[]; row: TData; expanded: boolean; onToggle: () => void }) {
  if (fields.length === 0) return null
  return (
    <div className="mt-2">
      {expanded && (
...
function EntityCardItem<TData extends Record<string, unknown>>({
  ...
  return (
    <div
      ...
      onKeyDown={onClick ? (e) => { if (e.key === 'Enter') onClick(row) } : undefined}
    ...
    >
      ...
      <div className={cn("p-4", indicator && indicator.edge !== 'right' && "pl-5", indicator && indicator.edge === 'right' && "pr-5")}>
        ...
        <CardOptionalSlot fields={slotGroups.optional} row={row} expanded={expanded} onToggle={() => setExpanded(prev => !prev)} />
      </div>
    </div>
  )
}
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-3">
        {Array.from({ length: loadingCount }).map((_, i) => <CardSkeleton key={`skeleton-${i}`} />)}
      </div>
    )
  }
