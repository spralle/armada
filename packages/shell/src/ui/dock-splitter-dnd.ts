...
import { type DockNode, readDockSplitRatio, setDockSplitRatio } from "../context-state.js";
...
export function wireDockSplitterDrag(root: HTMLElement, runtime: ShellRuntime, deps: DockSplitterDeps): void {
  ...
  for (const splitter of root.querySelectorAll<HTMLElement>(
    "[data-dock-splitter='true'][data-dock-split-id][data-dock-orientation]",
  )) {
...
function readSplitRatioByIdOrNull(node: DockNode, splitId: string): number | null {
  ...
  return readSplitRatioByIdOrNull(node.first, splitId) ?? readSplitRatioByIdOrNull(node.second, splitId);
}
