...
describe("deriveContractFromPackageJson", () => {
  it("derives correct namespace from scoped package name", () => {
    ...
    assert.equal(result.pluginId, "@ghost/vessel-view-plugin");
  ...
  it("returns all fields when all are present", () => {
    ...
    assert.deepStrictEqual(result, {
      pluginId: "@ghost/vessel-view-plugin",
  ...
  it("converts hyphens in package name to camelCase namespace", () => {
    const result = deriveContractFromPackageJson({
      name: "@ghost/theme-default-plugin",
      version: "1.0.0",
    });
    assert.equal(result.namespace, "ghost.themeDefault");
  });
  ...
  it("handles ghost field with no configNamespace", () => {
    const result = deriveContractFromPackageJson({
      name: "@ghost/nav-plugin",
      ghost: {},
    });
    assert.equal(result.namespace, "ghost.nav");
    assert.equal(result.version, "0.0.0");
  });
});
