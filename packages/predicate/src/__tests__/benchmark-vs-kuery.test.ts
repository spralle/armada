...
function makeMedium(i: number) {
  return {
    ...
    metadata: { created: `2024-01-${String(1 + (i % 28)).padStart(2, '0')}`, updated: `2024-06-${String(1 + (i % 28)).padStart(2, '0')}`, version: i % 20 },
  };
}
...
function makeLarge(i: number) {
  return {
    ...makeMedium(i),
    orders: [
      { id: i * 10, amount: (i % 500) + 10, items: [{ sku: `SKU${i % 100}`, qty: (i % 5) + 1 }, { sku: `SKU${(i + 7) % 100}`, qty: (i % 3) + 1 }] },
      { id: i * 10 + 1, amount: (i % 300) + 5, items: [{ sku: `SKU${(i + 20) % 100}`, qty: (i % 4) + 1 }] },
    ],
    preferences: { notifications: { email: i % 2 === 0, sms: i % 3 === 0 }, theme: i % 2 === 0 ? 'dark' : 'light', language: i % 3 === 0 ? 'en' : 'es' },
    history: [
      { action: 'login', timestamp: `2024-06-${String(1 + (i % 28)).padStart(2, '0')}T10:00:00Z`, details: { ip: `192.168.${i % 256}.${(i + 1) % 256}`, userAgent: 'Mozilla/5.0' } },
      { action: 'purchase', timestamp: `2024-06-${String(1 + (i % 28)).padStart(2, '0')}T12:00:00Z`, details: { ip: `10.0.${i % 256}.${(i + 2) % 256}`, userAgent: 'Chrome/125' } },
    ],
  };
}
...
interface BenchResult { avgMs: number; opsPerSec: number }
...
function runComparison(
  ...
  if (mode === 'cold') {
    predResult = bench(() => { find(collection, query); }, iters);
    kueryResult = bench(() => { new Kuery(query).find(collection); }, iters);
  } else {
    ...
    kueryResult = bench(() => { kq.find(collection); }, iters);
  }
    ...
    for (const [docSize, factory] of Object.entries(DOC_FACTORIES)) {
      ...
      describe(`docs: ${docSize}`, () => {
        for (const size of COLLECTION_SIZES) {
          test(`${size.toLocaleString()} items — cold`, () => {
...
describe('query: $elemMatch', () => {
  for (const size of COLLECTION_SIZES) {
    test(`large/${size.toLocaleString()} items — cold`, () => {
