...
describe('kuery compatibility', () => {
  ...
  it('should return correct element for property $eq query', () => {
    expect(
      new Predicate({ name: { $eq: 'Emil' } }).findOne(collection).name,
    ).toBe('Emil');
  });
  ...
  it('should return correct elements for property in query', () => {
    expect(
      new Predicate({ id: { $in: [1, 2] } }).find(collection).length,
    ).toBe(2);
  });

  it('should return correct elements for property in $nin query', () => {
    expect(
      new Predicate({ id: { $nin: [1, 2] } }).find(collection).length,
    ).toBe(3);
  });

  it('should return all elements for property with empty $nin query', () => {
    expect(
      new Predicate({ id: { $nin: [] } }).find(collection).length,
    ).toBe(5);
  });

  it('should return correct elements for property with path eq query', () => {
    expect(
      new Predicate({ 'address.street': 'Bellmansgatan' }).find(collection)
        .length,
    ).toBe(1);
  });
  ...
  it('should return correct elements for property with path ne query', () => {
    expect(
      new Predicate({ 'address.street': { $ne: 'Bellmansgatan' } }).find(
        collection,
      ).length,
    ).toBe(4);
  });
  ...
  it('should return correct elements for property with path in query', () => {
    expect(
      new Predicate({ name: { $in: ['Andreas', 'Emil'] } }).find(collection)
        .length,
    ).toBe(2);
  });
  ...
  it('should return correct elements for property $nin with strings query', () => {
    expect(
      new Predicate({ name: { $nin: ['Andreas', 'Emil'] } }).find(collection)
        .length,
    ).toBe(3);
  });
  ...
  it('should return correct elements for composite query (in + eq)', () => {
    expect(
      new Predicate({ name: { $in: ['Andreas', 'Emil'] }, id: 1 }).find(
        collection,
      ).length,
    ).toBe(1);
  });
  ...
  it('should return correct elements for composite query (nin + eq)', () => {
    expect(
      new Predicate({ name: { $nin: ['Andreas', 'Emil'] }, id: 2 }).find(
        collection,
      ).length,
    ).toBe(1);
  });
  ...
  it('should return correct elements for composite query (nin + eq, no match)', () => {
    expect(
      new Predicate({ name: { $nin: ['Andreas', 'Emil'] }, id: 1 }).find(
        collection,
      ).length,
    ).toBe(0);
  });
  ...
  it('should return correct elements for regex string query', () => {
    expect(
      new Predicate({ name: { $regex: 'Andr.*', $options: 'i' } }).find(
        collection,
      ).length,
    ).toBe(1);
  });
  ...
  it('should return correct elements for inline regex query', () => {
    expect(
      new Predicate({ name: /andr.*/i }).find(collection).length,
    ).toBe(1);
  });
  ...
  it('should return correct elements for property with path eq query with arrays', () => {
    expect(
      new Predicate({ 'girlfriends.name': 'eve' }).find(collection).length,
    ).toBe(1);
  });

  it('should return correct element for property gte query', () => {
    expect(
      new Predicate({ id: { $gte: 2 } }).find(collection).length,
    ).toBe(4);
  });

  it('should return correct element for property lte query', () => {
    expect(
      new Predicate({ id: { $lte: 2 } }).find(collection).length,
    ).toBe(2);
  });

  it('should return correct element for property gt query', () => {
    expect(
      new Predicate({ id: { $gt: 2 } }).find(collection).length,
    ).toBe(3);
  });

  it('should return correct element for property lt query', () => {
    expect(
      new Predicate({ id: { $lt: 2 } }).find(collection).length,
    ).toBe(1);
  });

  it('should return correct element for property lte date query', () => {
    expect(
      new Predicate({ born: { $lte: new Date('1981-01-01') } }).find(
        collection,
      ).length,
    ).toBe(1);
  });
  ...
  it('should return correct element for property gte date query', () => {
    expect(
      new Predicate({ born: { $gte: new Date('1981-01-01') } }).find(
        collection,
      ).length,
    ).toBe(4);
  });
  ...
  it('should return elements where given element exists is true', () => {
    expect(
      new Predicate({ address: { $exists: true } }).find(collection).length,
    ).toBe(2);
  });
  ...
  it('should return elements where given element exists is false', () => {
    ...
    expect(
      new Predicate({ girlfriends: { $exists: false } }).find(collection)
        .length,
    ).toBe(1);
  });
  ...
  it('should return elements where given element exists deeply', () => {
    expect(
      new Predicate({ 'girlfriends.wife': { $exists: true } }).find(collection)
        .length,
    ).toBe(2);
  });
  ...
  it('should return elements where given element does not exists deeply', () => {
    expect(
      new Predicate({ 'address.zipcode': { $exists: false } }).find(collection)
        .length,
    ).toBe(5);
  });
  ...
  it('should return elements when query for boolean', () => {
    expect(
      new Predicate({ isActive: true }).find(collection).length,
    ).toBe(2);
  });
  ...
  it('$or should do implicit and on subqueries', () => {
    expect(
      new Predicate({
        $or: [
          { 'girlfriends.name': 'Hanna', 'girlfriends.hotness': 10 },
