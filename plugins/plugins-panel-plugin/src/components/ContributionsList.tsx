...
export function ContributionsList({ contributions }: ContributionsListProps) {
  ...
  return (
    <div>
      <h4 className="font-semibold mb-1 text-xs text-foreground">Contributions</h4>
      <div className="flex flex-col gap-1 @md:flex-row @md:flex-wrap @md:gap-2">
        {nonEmpty.map((group) => (
          <div key={group.label} className="flex items-start gap-1.5">
            ...
            <span className="text-[11px] leading-tight text-muted-foreground">{group.items.join(", ")}</span>
          </div>
        ))}
      </div>
    </div>
...
function buildContributionGroups(c: PluginContributionsSummary): ContributionGroup[] {
  return [
    ...
    {
      label: "Keybindings",
      count: c.keybindings.length,
      items: c.keybindings.map((k) => `${k.keybinding} → ${k.action}`),
    },
    ...
    { label: "Services", count: c.services.length, items: c.services.map((s) => `${s.id} v${s.version}`) },
    {
      ...
      items: c.components.map((comp) => `${comp.id} v${comp.version}`),
    },
  ];
}
