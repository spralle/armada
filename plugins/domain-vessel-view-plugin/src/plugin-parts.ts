import type {
  ...
  JsonFormLayoutNode,
  JsonFormSchema,
} from "@ghost-shell/contracts/capabilities";
...
type MountContext = {
  readonly runtime: {
    readonly registry: {
      readonly resolveComponentCapability: (requesterPluginId: string, capabilityId: string) => Promise<unknown | null>;
    };
  };
};
...
const mountVesselViewPart: MountPart = async (target, context) => {
  const [jsonformCapability, entityListCapability] = await Promise.all([
    context.runtime.registry.resolveComponentCapability(REQUESTER_PLUGIN_ID, JSONFORM_COMPONENT_CAPABILITY),
    context.runtime.registry.resolveComponentCapability(REQUESTER_PLUGIN_ID, ENTITY_LIST_COMPONENT_CAPABILITY),
  ]);
...
function mountJsonForm(capability: unknown, host: HTMLElement): JsonFormController | null {
...
function renderCapabilityHost(target: HTMLElement, label: string, capability: unknown): HTMLElement {
